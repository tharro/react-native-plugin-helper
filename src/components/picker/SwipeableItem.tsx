...
function SwipeableItem<T>(
  ...
  const onAnimationEnd = useCallback(
    (_openDirection: OpenDirection, snapPoint: number) => {
  ...
  const openLeft: OpenPromiseFn = useCallback(
    (snapPoint, options) => {
    ...
    [animStatePos, maxSnapPointLeft, onAnimationEnd, springConfig]
  );

  const openRight: OpenPromiseFn = useCallback(
    (snapPoint, options) => {
    ...
    [animStatePos, maxSnapPointRight, onAnimationEnd, springConfig]
  );

  const close: ClosePromiseFn = useCallback(
    (options) => {
      const toValue = 0;
      return new Promise<void>((resolve) => {
        /**
    ...
    [animStatePos, onAnimationEnd, springConfig]
  );

  useImperativeHandle(ref, () => {
    const refObject: SwipeableItemImperativeRef = {
      open: (openDirections, snapPoint, options) => {
        if (openDirections === OpenDirection.LEFT) {
          return openLeft(snapPoint, options);
        }
        if (openDirections === OpenDirection.RIGHT) {
        ...
        const openDirections =
          closestSnapPoint === 0
            ...
            : closestSnapPoint > 0
              ? OpenDirection.RIGHT
              : OpenDirection.LEFT;
        runOnJS(onAnimationEnd)(openDirections, Math.abs(closestSnapPoint));
      };
      if (animStatePos.value === closestSnapPoint) {
        onComplete();
      } else {
        animStatePos.value = withSpring(
          closestSnapPoint,
  ...
  }, [percentOpenLeft, openLeft, sharedParams]);
